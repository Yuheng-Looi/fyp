        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=2.0) as response:
                if response.status == 200:
                    ...
                    self.logger.error(f"[FastAPI API Error] HTTP {response.status}: {response.reason}")
                    action  = "ALLOW"
                    verdict = "BENIGN"
        except urllib.error.HTTPError as e:
            self.logger.error(f"[FastAPI API Error] HTTP {e.code}: {e.reason}")
            action  = "ALLOW"
            verdict = "BENIGN"
        except Exception as e:
            ...
            if not (is_monitor or is_iperf):
                ...
                # --- OBSERVING ---
                    ...
                    if tot_fwd_pkts >= 3:
                        ...
                        # Install temporary optimistic forwarding rule (priority 60) to stop packet-in storm
                        if out_port != ofproto.OFPP_FLOOD and is_ip and ip_src and ip_dst:
                            ...
                            if proto == 6:
                                ...
                                if sport: match_fields['udp_src'] = sport
                                if dport: match_fields['udp_dst'] = dport
                            self.add_flow(datapath, 60, parser.OFPMatch(**match_fields), actions, idle_timeout=5)
