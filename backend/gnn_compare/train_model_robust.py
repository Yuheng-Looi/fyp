#!/usr/bin/env python3
"""
train_model_robust.py — Train Binary GNN with RobustScaler (15 features)

Trains a GAT-based binary GNN classifier using sklearn RobustScaler
...
Output:
    model_robust_binary_model.pt
    model_robust_binary_config.json
"""
...
def main():
    ...
    # 2. Fit RobustScaler on the 15 features using benign traffic only
    ...
    scaler_path = os.path.join(scaler_dir, "robust_scaler_15feat.pkl")
    joblib.dump(scaler, scaler_path)
    print(f"RobustScaler saved to {scaler_path}")

    # 3. Transform continuous features in-place
    print("Transforming continuous features using RobustScaler...")
    ...
    print("\n--- Running RobustScaler Model: Binary Classification ---")
    binary_config = {
        'hidden_dim': 64,
        'num_layers': 2,
        'dropout': 0.3
    ...
    run_experiment(
        ...
        split=(0.8, 0.1, 0.1),
        output_prefix='model_robust_binary'
    )

    print("\nRobustScaler Model Training Finished!")
