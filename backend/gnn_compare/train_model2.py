...
def main():
    # 1. Load raw 51-feature GNN dataset
    df, feature_cols, label_encoder = load_gnn_data_raw(
        cleaned_ref_path='01eda/cleaned_data52.csv',
        raw_folder='datasets',
    ...
    print("\n--- Running Model 2: Binary Classification ---")
    binary_config = {
        'hidden_dim': 64,
        'num_layers': 2,
        'dropout': 0.3
    ...
    print("\n--- Running Model 2: Multiclass Classification ---")
    multiclass_config = {
        'hidden_dim': 128,
        'num_layers': 3,
        'dropout': 0.5
    ...
    run_experiment(
        ...
        split=(0.7, 0.15, 0.15),
        output_prefix='model2_multiclass'
    )
    
    print("\nModel 2 Experiments Finished!")
